...
<br> 1. This is performed in dataSet1.R file. This can be found in line 7 and line 8 of the file.
<br> 2. The cumulative daily returns of Apple is `r round(100*sum(StockReturns[,2]),1)` %.
<br>    The average daily returns of Apple is `r round(100*mean(StockReturns[,2]),3)`%.
<br>    The standard deviation of the daily returns for the Apple is `r round(100*sd(StockReturns[,2]),3)`%. 
<br> 3. We will add YHOO in the line my tickers in dataSet1.r
<br> The cumulative daily returns of Yahoo is `r round(100*sum(StockReturns[,3]),1)`%. 
<br> The average daily returns of Yahoo is `r round(100*mean(StockReturns[,3]),3)`%. 
<br> The standard deviation of the daily returns for the Yahoo is `r round(100*sd(StockReturns[,3]),3)`%.
...
**Your Answers here:**
<br> 1. nrow and ncolumns commands should be used.`r transposedData = t(StockReturns)`
<br> The number of rows are `r nrow(transposedData)`.
<br> The number of columns are `r ncol(transposedData)`.
<br> 2. 
{r echo=TRUE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig.height=4,fig.width= 6, fig=TRUE}
...
**Your Answers here:**
<br> 1. In the DataSet1.Rdata file, we put start date of the analysis. Right now, the program runs with 2001-01-01.  
<br> 2. As done previously for the Apple by adding APPL, one can include more comapnies with correct nomenclature by looking at the Yahoo website.
<br> <br> 1. This returns 20 as output which is nothing but number of data-points read. 
{r echo=TRUE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig.height=4,fig.width= 6, fig=TRUE}
write.csv(StockReturns[1:20,c("SPY","AAPL")], file = "twentydays.csv", row.names = TRUE, col.names = TRUE) 
myData <- read.csv(file = "twentydays.csv", header = TRUE, sep=";")
...
<br> 2. It throws an error because format of myData doesn't match up with the one of StockReturns.
<br>
<br>
<br>
<br>
...
<br>### Creating Interactive Documents
